...
function FBConnect() {
    ...
    async function saveMessage2(messageText) {
        const var1=getAuth().currentUser.email;
        try {
            const db = getFirestore(app);
            await addDoc(doc(db, "cities"), {
                name: "Los Angeles",
                state: "CA",
                country: "USA"
              });
  ...
  return (
    <div className="wrapper">
        <div>FBConnect
            <button onClick={startThings}>Start things</button>
        </div>
        <div>
            <button onClick={()=>saveMessage2('test3')}>Send message</button>
        </div>
    </div>
  )
}
