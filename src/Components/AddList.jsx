...
function AddList(props) {
    ...
    async function validate() {
        if(newList.title==='') {
            ...
            props.parentProps.createList(newList);
            const tempData= await props.parentProps.getLists();
            props.parentProps.setLists(tempData);
            props.parentProps.toggleVisbility();
        }
    }
    useEffect(()=>{
        let temp=[...props.parentProps.lists];
        temp=temp.map(x=><div key={x.id} className="listName">{x.title}</div>);
        setExistingLists(temp);
    },[props.parentProps.lists]);


    return (
        <div className="modalbody">
        <div className="cancel" onClick={props.parentProps.toggleVisbility}>
